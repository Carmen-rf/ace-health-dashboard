...
# Row 1: ACE score by gender + by age
...
with col1:
    ...
    fig1 = px.bar(
        gender_ace, x="Gender", y="Mean ACE Score",
        color="Gender",
...
with col2:
    ...
    fig2 = px.line(
        age_ace, x="Age Group", y="Mean ACE Score",
        markers=True,
...
# Row 2: ACE category by income + education
col1, col2 = st.columns(2)

with col1:
    st.markdown("#### ACE Category by Income Level")
    income_order = ["<$15k", "$15-25k", "$25-35k", "$35-50k", "$50-100k", "$75-100k", "$100-200k", "$150-200k", "$200k+"]
    income_ace = filtered.groupby(["INCOME3", "ACE_CATEGORY"]).size().reset_index(name="Count")
    income_ace = income_ace[income_ace["INCOME3"].isin(income_order)]
    fig3 = px.bar(
        income_ace, x="INCOME3", y="Count", color="ACE_CATEGORY",
        barmode="stack",
        category_orders={"INCOME3": income_order},
        color_discrete_sequence=["#4A0E3A", "#9B2D6F", "#D4739A", "#F2C4D0"],
        labels={"INCOME3": "Income Level", "ACE_CATEGORY": "ACE Category"}
    )
    fig3.update_layout(plot_bgcolor="white", xaxis_tickangle=-45)
    st.plotly_chart(fig3, use_container_width=True)

with col2:
    st.markdown("#### ACE Category by Education Level")
    edu_order = ["No School", "Elementary", "Some High School", "High School Graduate", "Some College", "College Graduate"]
    edu_ace = filtered.groupby(["EDUCA", "ACE_CATEGORY"]).size().reset_index(name="Count")
    edu_ace = edu_ace[edu_ace["EDUCA"].isin(edu_order)]
    fig4 = px.bar(
        edu_ace, x="EDUCA", y="Count", color="ACE_CATEGORY",
        barmode="stack",
        category_orders={"EDUCA": edu_order},
        color_discrete_sequence=["#4A0E3A", "#9B2D6F", "#D4739A", "#F2C4D0"],
        labels={"EDUCA": "Education Level", "ACE_CATEGORY": "ACE Category"}
    )
...
# Row 3: BMI by ACE category + ACE score by gender/age heatmap
col1, col2 = st.columns(2)
...
with col1:
    ...
    fig5 = px.bar(
        bmi_ace, x="_BMI5CAT", y="Count", color="ACE_CATEGORY",
        barmode="group",
